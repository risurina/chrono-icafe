...
## Status: Complete (archived 2026-09-06)

All five phases implemented, verified, and committed on `feature/tenant-experience-v2`
in `.ai/worktree/tenant-experience-v2`:

- **Phase 1a** — `eec8a9ec` — dynamic open/closed status: schema (`hoursConfig` jsonb),
  `computeOpenStatus()`, `/public/venue-info` read path, landing render. Verified:
  `pnpm typecheck`, `rls:proof` (PASS), `pnpm build` all green; e2e spec added
  (`apps/chrono-web/e2e/tests/venue-info/dynamic-hours.spec.ts`) — correctness confirmed
  manually against live servers due to a **pre-existing** Playwright `page.request`
  flake in this suite (reproduces on already-existing, untouched specs too — not a
  regression from this plan).
- **Phase 1b** — `1e696cce` — dashboard hours editor UI on the branch settings page,
  wired to the existing `PATCH /branches/:id`. Verified: `typecheck`/`build` green;
  happy-path e2e passed live; the role-gate e2e case is blocked by this worktree's
  `EMAIL_PROVIDER=mailtrap` config (invite-link scraping needs `console`) — confirmed
  via curl + code review that the permission gate itself is correct and unchanged.
- **Phase 2** — `5d85c825` — SEO JSON-LD (`EntertainmentBusiness`), tenant-only,
  omits absent fields. Verified: `typecheck`/`build` green; real JSON-LD extracted from
  a live tenant render and confirmed absent on apex. Google's Rich Results Test itself
  was not run (no internet access from the implementation environment) — **needs a
  human pass**.
- **Phase 3** — `b17afda9` — `source` attribution (`?source=`, sessionStorage
  persistence, `?source=global_discovery` from `/discover`). Verified: `typecheck`/
  `build` green; all four acceptance scenarios observed directly in a real browser
  console (venue_qr, direct, global_discovery, downstream propagation).
- **Phase 4** — `b36e4f5c` — host-aware `sitemap.ts`/`robots.ts`. Verified:
  `typecheck`/`build` green; manually checked against live servers and cross-checked
  against DB state. Known limitation (disclosed in-code): the tenant list reuses
  `/public/discover/businesses` with a wildcard query since this phase was locked to
  chrono-web-only files — that endpoint caps at 20 results, so a deployment with more
  than 20 published tenants will under-list (never over-list) until a dedicated
  enumeration endpoint is built.

**Not implemented (by design):** Gap 4 (tenant success dashboard) — remains deferred,
blocked on the open analytics-vendor question below, which was never decided during
this implementation (correctly left to the developer).

**Follow-ups a human should still do:**
1. Run the Phase 2 JSON-LD block through Google's Rich Results Test.
2. Re-run the Phase 1a/1b role-gate e2e specs in an environment with
   `EMAIL_PROVIDER=console` to get full automated coverage of those two cases.
3. Decide the open analytics-vendor question below before ever picking Gap 4 back up.
