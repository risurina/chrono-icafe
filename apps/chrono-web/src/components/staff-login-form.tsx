...
/**
 * Staff / admin (Better Auth) sign-in form, shared by the apex `/login` (no
 * tenant context yet — org selection / creation) and a tenant host's
 * `/admin/login` (this business's back office). `resolveLandingUrl()` picks
 * the destination for both, so the two surfaces never disagree.
 */
export function StaffLoginForm({ customerLoginHref }: { customerLoginHref: string }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // a tenant host, else this business's back office, else (apex) the first
    // business's back office or business creation.
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Are you a customer?{" "}
            <Link href={customerLoginHref} className="text-primary hover:underline">
              Customer sign in
            </Link>
          </p>
        </CardContent>
