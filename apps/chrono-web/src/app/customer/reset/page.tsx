...
/** Global customer reset page — consumes the token from the emailed link. */
export default function CustomerResetPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    const { error } = await customerAuth.reset({ token, password });
    if (error) {
      toast.error(error);
      setLoading(false);
      return;
    ...
    setDone(true);
    setTimeout(() => (window.location.href = "/customer/login"), 900);
  }
